...
export default function TabBonHeThong({ tomTat, onTaiLai, dangTai }) {
  if (!tomTat) {
  ...
  <>
    ...
    {tomTat.heThong.map((ht) => {
      const pct = Math.round((ht.soKey / maxKey) * 100);
      const mau = MAU_TRANG_THAI[ht.trangThai] || MAU_TRANG_THAI.TRONG;
      return (
        <View key={ht.id} style={styles.card}>
          <View style={{ flexDirection: 'row', gap: 14 }}>
            <Text style={{ fontSize: 32 }}>{ht.icon}</Text>
            <View style={{ flex: 1 }}>
              <Text style={styles.tenCard}>{ht.ten}</Text>
              <Text style={styles.meta}>{ht.moTa}</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 12 }}>
                <Text style={[styles.meta, { fontWeight: '700', color: mau }]}>
                  {ht.trangThai === 'CO_DU_LIEU' ? '● Có dữ liệu' : '○ Trống'}
                </Text>
                <Text style={styles.meta}>{ht.soKey} keys · {ht.kichThuocHienThi}</Text>
              </View>
              <View style={styles.thanhTienDo}>
                <View style={[styles.thanhTienDoFill, { width: `${pct}%`, backgroundColor: mau }]} />
              </View>
            </View>
          </View>
        </View>
