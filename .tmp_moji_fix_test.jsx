/**
 * PH�N H?: TR?M GI�M �?NH & S?A L?I XML (PHUONG CH�U - JCI)
 * N�ng c?p 5.0: 
 * 1. �?c h�ng lo?t file XML c�ng l�c v� duy?t b?ng Tab ngang.
 * 2. Layout chia d�i m�n h�nh: Lu?i d? li?u (tr�i) - S? tay l?i (ph?i) gi�p xem full n?i dung.
 * 3. T?i uu UI chu?n Phuong Ch�u: �ua to�n b? Tabs v� N�t ch?c nang l�n G�C TR�N PH?I, c�ng k�ch thu?c.
 * 4. D�n d�ng, d�n c?t linh ho?t, t? d?ng bao tr?n Text kh�ng b? che khu?t (Auto-Height).
 * 5. FIX L?I: S?a l?i typo (sai ch�nh t?) c�c tru?ng MA_BENH_CHINH, MA_TINH_CU_TRU theo chu?n Q� 130.
 * 6. FIX L?I TR�N B? NH?: T�ch h?p thu?t to�n Auto-Chunking d? bam nh? d? li?u khi luu v�o LocalStorage.
 */
...
// ============================================================================
// THU?T TO�N CH?NG TR�N B? NH? LOCAL STORAGE (CHUNKING)
// ============================================================================
const luuKhoHoSoChongTranBoNho = async (key, dataArray) => {
    try {
        ...
        // Bam nh? d? li?u v� luu t?ng m?ng con
        ...
        console.log(`�� luu th�nh c�ng ${dataArray.length} h? so, bam th�nh ${totalChunks} kh?i.`);
    } catch (e) { 
        console.error("L?i Chunking Set Storage:", e); 
        alert("L?i tr�n b? nh? chua th? x? l�. Vui l�ng th? n?p �t h? so hon.");
    }
};

// H�M �?C D? LI?U �� BAM T? LOCAL STORAGE GH�P L?I
const docKhoHoSoChongTranBoNho = async (key) => {
    try {
        ...
        console.error("L?i Chunking Get Storage:", e);
        return null;
    }
};
// ============================================================================

// �� S?A L?I CH�NH T? CHO KH?P 100% V?I TH? XML Q� 130
const CHUAN_COT_XML = {
  'XML1': [
...
const ManHinhDocXML = () => {
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        ...
        console.error("L?i kh�i ph?c session:", e);
      } finally {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(async () => {
      try {
        ...
        // 1. T? �?NG CH?Y L?I GI�M �?NH KHI D? LI?U THAY �?I
        const loiMoi = await chayBoMayGiamDinhV3(hoSoDangXem);

        // 2. C?P NH?T TR?NG TH�I L?I TR�N GIAO DI?N
        ...
        console.error("L?i Auto-save & Gi�m d?nh:", e);
      }
    }, 800);
  }, [hoSoDangXem]);
  ...
  const handleNhanDuLieu = async (dataList) => {
    if (dataList && dataList.length > 0) {
      ...
      for (const hoSo of dataList) {
          ...
          if (maLK && !dsMoMoi.some(hs => layMaLK(hs) === maLK)) {
  ...
  const handleQuetLai = async () => {
    ...
    Alert.alert("H? th?ng CDSS", `�� c?p nh?t gi�m d?nh: ${loi.length} l?i.`);
  };
  ...
  const handleXuatXML = () => {
    if (Platform.OS !== 'web') return Alert.alert("Th�ng b�o", "Ch?c nang xu?t file ch? h? tr? tr�n Web.");
    if (!hoSoDangXem) return;
    ...
    a.href = url;
    a.download = `SAU_GIAM_DINH_${layMaLK(hoSoDangXem) || 'EDITED'}.xml`;
    a.click();
  };

  const handleJumpToError = (index, truongLoi) => {
    if (!truongLoi || truongLoi === 'UNKNOWN') {
      Alert.alert("Th�ng b�o", "��y l� l?i c?u tr�c h? th?ng.");
      return;
    }
    if (mainScrollRef.current) {
        // T?a d? nh?y u?c t�nh an to�n khi d�ng Auto-Height
              ...
              <View key={rIdx} style={styles.data_row}>
                ...
                {cotHienThi.map((col, cIdx) => {
                  const loi = ketQuaLoiHienTai.find(l => 
                    layTabLoi(l).startsWith(tabHienTai) &&
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.top_nav}>
        <View style={styles.top_nav_left}>
           <TouchableOpacity onPress={() => navigation.goBack()} style={styles.back_btn}>
             <Text style={styles.back_txt}>? TR? V?</Text>
           </TouchableOpacity>
           <Text style={styles.main_title}>?? �?C XML CHI TI?T</Text>
        </View>
      ...
      {!hoSoDangXem ? (
        <View style={styles.welcome}>
          ...
          <Text style={styles.hint}>Nh?n ch?n (ho?c b�i den nhi?u) t?p XML. H? th?ng s? t? d?ng qu�t l?i.</Text>
        </View>
      ) : (
        <View style={styles.workspace}>
          ...
          <View style={styles.thanh_tong_hop}>
             <View style={styles.thong_tin_bn_khu_vuc}>
               <Text style={styles.p_name}>{layXml1(hoSoDangXem)?.HO_TEN || 'Kh�ng r� b?nh nh�n'} - {layMaLK(hoSoDangXem)}</Text>
               <Text style={styles.p_sub}>{layXml1(hoSoDangXem)?.CHAN_DOAN_RV || 'Kh�ng c� ch?n do�n ra'}</Text>
             </View>

             <View style={[styles.the_tong_hop, { borderLeftColor: '#D81B60' }]}>
                <Text style={styles.nhan_tong_hop}>T?NG L?I</Text>
                <Text style={styles.so_tong_hop}>{currentLoi.length}</Text>
             </View>
             <View style={[styles.the_tong_hop, { borderLeftColor: '#43A047' }]}>
                <Text style={styles.nhan_tong_hop}>S?A �U?C</Text>
                <Text style={[styles.so_tong_hop, { color: '#2E7D32' }]}>{loiSuaDuoc}</Text>
             </View>
             <View style={[styles.the_tong_hop, { borderLeftColor: '#D32F2F' }]}>
                <Text style={styles.nhan_tong_hop}>H? TH?NG</Text>
          ...
          <View style={styles.main_content_layout}>
             <View style={styles.sidebar_tab_trai}>
                <Text style={styles.sidebar_tab_title}>B?NG XML</Text>
             ...
             <View style={styles.audit_log_container_split}>
                <Text style={styles.audit_title}>?? S? TAY L?I</Text>
                <ScrollView style={styles.audit_list}>
                  {loiTabHienTai.map((l, i) => {
                    const canFix = l.truong_loi && l.truong_loi !== 'UNKNOWN';
                    return (
                      <TouchableOpacity key={i} style={styles.log_item} onPress={() => handleJumpToError(l.index || 0, l.truong_loi)} onLongPress={() => canFix && handleMoSuaHoSoBanSao(l)}>
                        <View style={[styles.log_tag, {backgroundColor: canFix ? '#43A047' : '#D32F2F'}]}>
                          <Text style={styles.log_tag_txt}>{canFix ? 'S?A �U?C' : 'H? TH?NG'}</Text>
                        </View>
                        <View style={styles.log_content}>
                          <Text style={styles.log_field}>Tru?ng: [{l.truong_loi || 'N/A'}] - D�ng {(l.index || 0) + 1}</Text>
                          <Text style={styles.log_msg}>{l.canh_bao}</Text>
                          {canFix && <Text style={styles.log_copy_hint}>Gi? l�u d? m? b?n sao ch?nh s?a v� luu phi�n b?n.</Text>}
                        </View>
                        {canFix && <Text style={styles.log_jump_icon}>??</Text>}
                      </TouchableOpacity>
                    );
                  })}
                  {loiTabHienTai.length === 0 && (
                    <Text style={styles.log_success_txt}>? Kh�ng c� l?i t?i b?ng n�y.</Text>
                  )}
                </ScrollView>
             </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  log_copy_hint: { fontSize: 16, color: CD.text.muted, marginTop: 6, fontStyle: 'italic' },
